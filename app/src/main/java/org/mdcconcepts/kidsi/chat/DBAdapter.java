...
public class DBAdapter {
	...
	public static final String KEY_USER_FROM = "user_from_name";
	
	public static final String KEY_USER_TO = "user_to_name";
	
	public static final String KEY_USER_MSGTYPE    = "user_msg_type";
	...
	private static final String USER_CREATE = "create table if not exists tbl_user(_id integer primary key autoincrement, user_from_name text not null, user_to_name text not null, user_message text not null,user_msg_type text not null);";
  ...
  /********************** Main Database creation INNER class ********************/
    ...
    public static void addUserData(UserData uData) {
    	try{
		    	...
		    	String from  = sqlEscapeString(uData.getFrom());
		    	String to  = sqlEscapeString(uData.getTo());
		    	String message  = sqlEscapeString(uData.getMessage());
				String messageType=sqlEscapeString(uData.getMessageType());
				ContentValues cVal = new ContentValues();
				cVal.put(KEY_USER_FROM, from);
				cVal.put(KEY_USER_TO, to);
				cVal.put(KEY_USER_MESSAGE, message);
				cVal.put(KEY_USER_MSGTYPE, messageType);
				long i=db.insert(USER_TABLE, null, cVal);
				Log.i("DBEntry 1",""+i);
		        db.close(); // Closing database connection
	    } catch (Throwable t) {
			Log.i("Database", "Exception caught: " + t.getMessage(), t);
...
//        		KEY_USER_FROM, KEY_USER_TO,KEY_USER_MESSAGE}, KEY_ID + "=?",
...
//    }
    ...
    public static List<UserData> getMessageUserData(String senderUsername,String receiverUsername) {
        List<UserData> contactList = new ArrayList<UserData>();
        // Select All Query
        String selectQuery = "SELECT  * FROM " + USER_TABLE+" WHERE ("+KEY_USER_FROM +"= '"+senderUsername+"' and "+KEY_USER_TO+"= '"+receiverUsername+"') or ("+KEY_USER_FROM +"= '"+receiverUsername+"' and "+KEY_USER_TO+"= '"+senderUsername+"') ORDER BY "+KEY_ID;
        Log.d("my message","my message");
        final SQLiteDatabase db = open();
        ...
        if (cursor.moveToFirst()) {
            do {
            	...
            	data.setFrom(cursor.getString(1));
            	data.setTo(cursor.getString(2));
            	data.setMessage(cursor.getString(3));
            	data.setMessageType(cursor.getString(4));
            	Log.d("my message","my message");
            	
            	//data.setSenderName(cursor.getString(4));
